...
interface ContextType {
    ...
    refreshData: () => Promise<void>; // Add refreshData here
...
export function ContextWrapper({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        if (userId) {
            refreshData(); // Use refreshData in useEffect to load both expenses and categories initially
        }
    }, [userId]);
    ...
    const addExpense = async (amount: number, description: string, categoryname: string, userId: string, date: string): Promise<void> => {
        ...
        try {
            ...
            refreshData(); // Refresh data after adding expense
        } catch (error) {
            toast({
                title: 'Error',
    ...
    const addCategory = async (name: string, userId: string) => {
        ...
        try {
            const response = await apiAddCategory(name, userId);
            if (response.status === 201) {
                ...
                toast({
                    title: "Success",
                    description: response ? response?.data?.message : 'Successfully Added Category',
                    variant: 'default'
                });
            }
            refreshData(); // Refresh data after adding category
        } catch (error) {
            toast({
                title: 'Error',
                description: `${(error as { response?: { data?: { message?: string } } })?.response?.data?.message}`,
    ...
    const handleUpdate = async (amount: number, description: string, userId: string, categoryId: string, date: string) => {
        try {
            ...
            if (expenseId) {
                ...
                refreshData(); // Refresh data after updating expense
            }
        } catch (error: any) {
            toast({
                variant: 'destructive',
    ...
    const handleDelete = async () => {
        ...
        try {
            if (expenseId) {
                ...
                refreshData(); // Refresh data after deleting expense
            }
        } catch (error: any) {
            toast({
                variant: 'destructive',
    ...
    return (
        <AppContext.Provider value={{ session, expenseData, categories, isExpenseLoading, isCategoryLoading, error, userId, isExpensePending, isCategoryPending, saving, deleting, expenseId, addCategory, addExpense, getTotalExpense, handleUpdate, handleDelete, refreshData, setExpenseId }}>
            {children}
        </AppContext.Provider>
    );
}
...
//     return (
//         <AppContext.Provider value={{ session, expenseData, categories, isExpenseLoading, isCategoryLoading, error, userId, isExpensePending, isCategoryPending, saving, deleting, expenseId, addCategory, addExpense, getTotalExpense, handleUpdate, handleDelete, setExpenseId, refreshData }}>
//             {children}
//         </AppContext.Provider>
//     );
// }
